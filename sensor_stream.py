'''
This class streams data from APDM Opal sensors.

NOTE: Configuration of the sensor must be completed before
this class can be used. Refer to the Getting Started guide
in this repository or to APDM's official SDK documentation.
'''
import apdm
...
class SensorStream:
    ...
    def get_next(self):
        ...
        Returns:
            A 2D list, one row for each active sensor. Each row has the following data:
                - Computer Unix time in milliseconds
                - Sensor Unix time in milliseconds
    ...
    def _get_next_record_from_sensor(self):
        '''
        Get next raw record from sensor
        '''
    ...
    def _raw_record_to_gyro_accel_and_mag(self, raw_record):
        '''
        Format raw record into a list of:
            - Computer's Unix time in milliseconds
            - APDM Sensor's Unix time in microseconds
            - Device/sensor serial number
            - Accelerometer X, Y, and Z
            - Gyroscope X, Y, and Z
            - Magnetometer X, Y, and Z
        ...
        for device_id in self.device_ids:
            ...
            single_device_data = [
                        ...
                        str(raw_record.v2_sync_val64_us), # APDM sensor's Unix time in microseconds
